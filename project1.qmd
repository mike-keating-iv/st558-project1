...
plot.state <- function(df, var_name = "Enrollment"){
  ...
  df |> 
    ...
    mutate(division = as.factor(division)) |> # TODO: Make this factor when reading data??
    # Plotting functions
...
plot.county <- function(df, var_name = "Enrollment", 
  ...
  counties <- df |> 
    filter({{state}} == state ) |>
    group_by(area_name) |> 
    summarize(mean_enrollment = mean(get(var_name))) |> display_function(mean_enrollment, top_or_bottom) |> head(n)
...
```{r}
# Test Functions
plot(county_tibble, top_or_bottom = "bottom", n = 10, state = "SC")
```
