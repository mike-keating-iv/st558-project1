...
We can look up state abbreviations/names using built in R constants, and filter columns by the state in our divisions tibble by using if_any and str_detect.

Source: [StackOverflow](https://stackoverflow.com/questions/5411979/state-name-to-abbreviation)

```{r}
# TODO: Assign division based on the state-division pairs we read in
# Let's make a function to make this easier

get_division <- function(state_abb){
  # Get the state name for abbreviation
  ...
  division <- divisions |> filter(if_any(States, ~str_detect(.x, state_name)))
